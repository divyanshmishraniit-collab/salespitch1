...
function DiffBadge({ level }) {
  const map = {
    Beginner:     { color: '#6EE7B7', label: 'Beginner' },
    Intermediate: { color: '#FCD34D', label: 'Intermediate' },
    Advanced:     { color: '#FCA5A5', label: 'Advanced' },
  };
    ...
    >
      ...
      <div className="scenario-card__footer">
        <div className="scenario-card__meta">
          ...
          <span className="scenario-card__time">
            <Clock size={11} />
...
const DEFAULT_SCENARIOS = [
  {
    id: 'sc-1',
    tag: 'Lead Generation',
    title: 'Cold Outreach: Skeptical IT Head',
  ...
  },
  {
    ...
    title: 'Budget Pushback from Finance Director',
  ...
  },
  {
    id: 'sc-3',
    tag: 'Upset Customer',
    title: 'Post-Training Dissatisfaction',
  ...
  {
    id: 'sc-4',
    tag: 'Competitive Sales',
    title: 'Competing Against a Global Brand',
  ...
  },
  {
    id: 'sc-5',
    tag: 'Deal Closure',
    title: 'Final Push Before Quarter Close',
...
export default function TutorialPage() {
  ...
  /* ──────────────────────────────────────────────────────
     callLiteLLM — thin wrapper around the new backend route
     POST /api/tutorial-ai  { systemPrompt, userContent }
     ────────────────────────────────────────────────────── */
  const callLiteLLM = async (systemPrompt, userContent) => {
    ...
    return data.result; // plain string returned by LiteLLM
  };
...
The salesperson just responded to the customer scenario above.

Your job:
1. Evaluate how well they handled the situation
2. Be constructive but honest
3. Structure your feedback clearly
...
Keep it concise, practical, and specific to GenAI Training sales.`;

      const feedback = await callLiteLLM(systemPrompt, userText);
      setMessages(prev => [
        ...prev,
        { role: 'assistant', isFeedback: true, text: feedback },
      ]);
      setAttemptDone(true);
    } catch (err) {
      setError('Could not get feedback — ' + err.message);
    } finally {
      setIsLoading(false);
    }
  };
...
Existing scenarios (do NOT repeat these): ${existingTitles}

IMPORTANT: Respond ONLY with valid JSON — no markdown fences, no explanation, just the raw JSON array.

[
  {
    "id": "sc-${ts}-1",
    "tag": "one of: Lead Generation, Deal Closure, Upset Customer, Objection Handling, Discovery Call, ROI Justification, Competitive Sales",
    "title": "Short compelling title",
    "context": "2-3 sentences setting the scene, ending with a specific challenge or statement the salesperson must respond to",
    "difficulty": "Beginner or Intermediate or Advanced",
    "estimatedTime": "X–Y min",
    "customerPersona": "One sentence describing the customer mindset and behavior"
  },
  {
    "id": "sc-${ts}-2",
    ...same shape...
...
All scenarios must be about selling the GenAI Training Program (cohort-based AI tool training for companies).`;
        ...
        <div className="practice-chat-wrap">
          <div className="practice-chat">
            {messages.map((msg, i) => (
              ...
              >
                {msg.isCustomer && (
                  <div className="practice-msg__avatar customer-avatar"><Users size={13} /></div>
                )}
                {msg.isFeedback && (
                  <div className="practice-msg__avatar feedback-avatar"><Sparkles size={13} /></div>
                )}

                <div className={`practice-msg__bubble ${
                  msg.isCustomer ? 'customer-bubble' : msg.isFeedback ? 'feedback-bubble' : 'user-bubble'
                }`}>
                  {msg.isCustomer && <div className="bubble-label">Customer</div>}
                  {msg.isFeedback && (
                    <div className="bubble-label feedback-label">
                      <Sparkles size={11} /> AI Coach Feedback
                    </div>
                  )}
                  {msg.isFeedback
                    ? <FeedbackDisplay text={msg.text} />
                    : <p>{msg.text}</p>
                  }
                </div>

                {!msg.isCustomer && !msg.isFeedback && (
                  <div className="practice-msg__avatar user-avatar">You</div>
                )}
              </div>
            ))}
            ...
            {isLoading && (
              <div className="practice-msg feedback">
                ...
                <div className="practice-msg__bubble feedback-bubble">
                  <div className="bubble-label feedback-label"><Sparkles size={11} /> AI Coach Feedback</div>
          ...
          {attemptDone ? (
            <div className="attempt-done-bar">
              ...
              <div className="attempt-done-actions">
                <button className="retry-btn" onClick={resetAttempt}>
                  <RotateCcw size={14} />
                  Try Again
                </button>
                <button className="newscene-btn" onClick={goBack}>
                  <BookOpen size={14} />
                  New Scenario
                </button>
              </div>
            ...
            <>
              ...
              <div className="practice-controls">
                {isRecording ? (
                  ...
                  <button className="ctrl-btn record" onClick={startRecording} disabled={isLoading}>
                    <Mic size={15} />
                    {transcript ? 'Record Again' : 'Record Your Response'}
                  </button>
                )}
                <button
                ...
                >
                  {isLoading ? <span className="mini-spinner" /> : <Send size={15} />}
              ...
              <p className="practice-hint">
                {isLoading
                  ...
                  : isRecording
                    ? '🎤 Recording — speak your response naturally, then stop'
                    : transcript
                      ? '✅ Ready — submit for AI feedback or record again'
                      : "🎯 Read the scenario above, then record how you'd respond to the customer"}
              </p>
            </>
          )}
        </div>
  ...
  return (
    <div className="tutorial-page">
      <div className="tutorial-hero animate-fadeUp">
        <div className="tutorial-hero__eyebrow">
          <Zap size={13} />
          <span>Interactive Training</span>
        </div>
        <h1 className="tutorial-hero__title">
          Sales Scenario
          <span className="tutorial-hero__accent"> Practice</span>
        </h1>
      ...
      <div className="how-it-works animate-fadeUp">
        {[
          ...
          <div key={i} className="how-step">
            <div className="how-step__icon"><step.icon size={18} /></div>
            <div>
        ...
        <div className="scenarios-grid">
          {scenarios.map((sc, i) => (
            <ScenarioCard
              ...
              isNew={newScenarioIds.has(sc.id)}
            />
          ))}
        </div>
          ...
          >
            {isGenerating ? (
              ...
              <>
                <Plus size={16} />
                Make More Scenarios
                <span className="generate-more-hint">+2 new</span>
              </>
            )}
          </button>
          <p className="generate-more-sub">
            AI will generate 2 fresh scenarios tailored to GenAI Training sales
          </p>
        </div>
      </div>
    </div>
